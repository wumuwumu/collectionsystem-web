    ...
    <div>
        <Card>
            ...
            <div>
                ...
                >
                    <FormItem label="用户账号：">
                        <span v-text="userForm.account"></span>
                    </FormItem>
                    ...
                    <FormItem label="微信：">
                        <div style="display:inline-block;width:300px;">
                            <Input v-model="userForm.weiXin"></Input>
                        </div>
                    </FormItem>
                    <FormItem label="qq：">
                        <div style="display:inline-block;width:300px;">
                            <Input v-model="userForm.qq"></Input>
                        </div>
                    </FormItem>
                    <FormItem label="邮箱：">
                        <div style="display:inline-block;width:300px;">
        ...
        <Modal v-model="editPasswordModal" :closable='false' :mask-closable=false :width="500">
            ...
            <Form ref="editPasswordForm" :model="editPasswordForm" :label-width="100" label-position="right"
                ...
                <FormItem label="原密码" prop="oldPassword" :error="oldPassError">
                    <Input v-model="editPasswordForm.oldPassword" placeholder="请输入现在使用的密码" type="password"></Input>
                </FormItem>
                <FormItem label="新密码" prop="newPassword">
                    <Input v-model="editPasswordForm.newPassword" placeholder="请输入新密码，至少6位字符" type="password"></Input>
                </FormItem>
                <FormItem label="确认新密码" prop="rePassword">
                    <Input v-model="editPasswordForm.rePassword" placeholder="请再次输入新密码" type="password"></Input>
                </FormItem>
            </Form>
            <div slot="footer">
    ...
    export default {
        ...
        methods: {
            ...
            saveEdit () {
                this.$refs['userForm'].validate((valid) => {
                    if (valid) {
                        this.$store.dispatch("ChangeUserPassword",this.userForm).then(result =>{
                            if(result.code ==1){
                                this.saveInfoAjax();
                            }
                            else {
            ...
            saveEditPass () {
                this.$refs['editPasswordForm'].validate((valid) => {
                    if (valid) {
                        ...
                        // you can write ajax request here
                        this.$store.dispatch("ChangePassword", this.editPasswordForm).then((result) => {
                            if (result.code == 1) {
            ...
            init () {
                this.$store.dispatch("GetInfo").then((result) => {
                    if (result.code == 1) {
                        ...
                        this.userForm.weiXin = result.data.weiXin;
                        this.userForm.qq = result.data.qq;
                        this.userForm.company = result.data.company;
                    } else {
